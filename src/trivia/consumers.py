...
import json
from trivia.api.serializers import TriviaCountSerializer, CommentCountSerializer
from trivia.models import Trivia, Comment


def get_trivia_data(triviaId):
    trivia = Trivia.objects.filter(id__in=triviaId)
    serializer = TriviaCountSerializer(trivia, many=True)
...
class TimelineConsumer(WebsocketConsumer):
    ...
    def fetch_trivia(self, data):
        updates = get_trivia_data(data['triviaId'])
        if updates:
            content = {
                'command': 'fetch_trivia',
                'update': updates
            }
