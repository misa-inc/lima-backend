...
# TODO Add more details to post, posts, comments
...
def post_for(instance, filename):
...
#TODO Add the all different types of post
class Post(models.Model):
    ...
    answer_two = models.CharField(max_length=300, blank=True, null=True)
    answer_three = models.CharField(max_length=300, blank=True, null=True)
    answer_four = models.CharField(max_length=300, blank=True, null=True)
    is_answer = models.CharField(max_length=300, blank=True, null=True)#can be one, two, three, four, five
    active_for = models.CharField(max_length=300, blank=True, null=True)#can be one, two, three, four, five, six, seven
    prize = models.CharField(max_length=300, blank=True, null=True)#prize for takin the quiz
    directory = models.ForeignKey(
        Directory, blank=True, null=True, on_delete=models.CASCADE, related_name="directory"
    )
    page = models.ForeignKey(
        Page, blank=True, null=True, on_delete=models.CASCADE, related_name="page"
    )
    parent = models.ForeignKey(
        "self", blank=True, null=True, on_delete=models.CASCADE, related_name="alt"
    )
    author = models.ForeignKey(
        'account.User', on_delete=models.CASCADE, related_name="post_author"
    )
    report = models.ManyToManyField(
        'account.User', related_name="post_report", blank=True, default=None
    )
    saved = models.ManyToManyField(
        'account.User', related_name="post_saved", blank=True, default=None
    )
    ...
    def new_answers(self):
        return self.answer_set.order_by("created")
    ...
    def save(self, *args, **kwargs):
        self.slug = slugify(self.post_type)
        new_record = self.pk is None
        with transaction.atomic():
            super(Post, self).save(*args, **kwargs)
            if new_record:
                Vote.objects.create(voter=self.author, value=1, post=self)
                assign_perm("post.change_post", self.author, self)
                assign_perm("post.delete_post", self.author, self)
...
class Answer(models.Model):
    ...
    report = models.ManyToManyField(
        'account.User', related_name="post_answer_report", blank=True, default=None
    )
    saved = models.ManyToManyField(
        'account.User', related_name="post_answer_saved", blank=True, default=None
    )
    parent_answer = models.ForeignKey(
        "self", null=True, on_delete=models.CASCADE, blank=True
    )
    question = models.ForeignKey(Post, on_delete=models.CASCADE, related_name="question")
    perspective = models.CharField(max_length=300, blank=True, null=True)
    is_deleted = models.BooleanField(default=False)
    has_perspective = models.BooleanField(default=False)
    saved_count = models.IntegerField(blank=True, null=True, default=0)
    ...
    voters = models.ManyToManyField(
        'account.User',
        related_name="post_answer_voters",
    )
    votes = models.IntegerField(default=0)
    ...
    @property
    def child_answers(self):
        return self.answer_set.all().order_by("-votes", "created")    
    ...
    def save(self, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            if new_record:
                ...
                assign_perm("post.change_answer", self.author, self)
                assign_perm("post.delete_answer", self.author, self)
...
class Comment(models.Model):
    ...
    author = models.ForeignKey(
        'account.User', on_delete=models.CASCADE, related_name="post_comment_author"
    )
    parent_comment = models.ForeignKey(
        "self", null=True, on_delete=models.CASCADE, blank=True
    )
    report = models.ManyToManyField(
        'account.User', related_name="post_comment_report", blank=True, default=None
    )
    saved = models.ManyToManyField(
        'account.User', related_name="post_comment_saved", blank=True, default=None
    )
    perspective = models.CharField(max_length=300, blank=True, null=True)
    created = models.DateTimeField(auto_now_add=True)
    text = models.TextField(blank=True, null=True)
    ...
    voters = models.ManyToManyField(
        'account.User',
        related_name="post_comment_voters",
    )
    votes = models.IntegerField(default=0)
    ...
    def save(self, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            if new_record:
                Vote.objects.create(voter=self.author, value=1, comment=self)
                assign_perm("post.change_comment", self.author, self)
                assign_perm("post.delete_comment", self.author, self)
...
class Vote(models.Model):
    voter = models.ForeignKey('account.User', related_name="post_voter", on_delete=models.CASCADE)
    ...
    def validate(self):
        ...
        if self.post is None and self.answer is None:
            raise FieldError("post and answer cannot both be null")
        if self.post and self.comment:
            raise FieldError("cannot submit vote for both post and comment")
        if self.post and self.answer:
            raise FieldError("cannot submit vote for both post and answer")
    ...
    def save(self, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            if self.comment:
                ...
                self.comment.save()
            if self.answer:
                value__sum = self.answer.vote_set.aggregate(Sum("value"))
                self.answer.votes = value__sum["value__sum"]
                self.answer.save()    
            if self.post:
